...
namespace Culture_ChatBot  // 도로명 주소로 검색 다이얼로그
{
    [Serializable]
    public class NameSelectDialog : IDialog<string>
    {
        string dummy = "";
                    ...
                    foreach (List<Dictionary<string, string>> content in contentList)
                    {
                        foreach (Dictionary<string, string> value in content)
                        {
                            ...
                            */
                            bool b;
                            string eventNm = value["eventNm"].Replace(" ", "");
                            b = eventNm.Contains(activity.Text.Trim().Replace(" ", ""));
                            if (b)
                            {
                                count++;
                                var message = context.MakeMessage();
                                message.Attachments.Add(CardHelper.GetHeroCard(value["eventNm"], value["eventCo"],
                                                                               value["eventStartDate"], value["eventEndDate"],
